...
router.get('/query-glyph-info', function(req, res, next) {
	const query = req.query['glyph-key'];

	// Reject empty queries;
	if(typeof query !== 'string' || query.length <= 0)
		return next(new HttpError(400, 'Query must not be empty.'));
	// Reject queries that are too long.
	if(query.length > 10)
		return next(new HttpError(400, 'Query too long.'));
	...
	// If not, reject.
	if(glyphInfo === null)
		return next(new HttpError(404, 'Glyph does not exist.'));
