...
//#region 字表行之類型定義
/**
 * @typedef {Object} KwangHjunAllCharactersTableRow
 * 廣韻全字表之一行數據。
...
/** @returns {Promise<KwangHjunAllCharactersTableRow[]>} */
async function ReadRowsFromTable(fileContent) {
	...
	for(var startIndex = 0, rowCount = 0; startIndex < totalLength; ++rowCount) {
		...
		// Skip the header row.
		if(rowCount != 0) {
			const row = Object.fromEntries(rowRaw.split(',').map((v, i) => [rowKeys[i], v]));
			rows.push(row);
		}
